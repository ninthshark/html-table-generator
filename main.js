...
function numberOfColumn() {
  ...
  for (var i = 0; i < column; i++) {
    ...
    document.querySelectorAll('.text-area')[i].setAttribute('contentEditable','true');
  }
  document.querySelector('.add-preview').insertAdjacentHTML('beforeend','<button class="add-row">+ Row</button>');
  document.querySelector('.add-preview').insertAdjacentHTML('beforeend','<button class="preview">Generate Html</button>');
...
function addRow() {
  ...
  //for (var i = 0; i < col; i++) {
    row.insertCell(i).innerHTML = '<div class="text-area">&nbsp;</div>';
    row.querySelectorAll('.text-area')[i].setAttribute('contentEditable','true');
  }
...
//function getHtmlTags() {
  ...
  var newContent = newContent.replace(/(<div\s*class="text-area"\s*contenteditable="true">)|(<\Wdiv>)/g, '');
  return newContent;
};
...
function toggleSelectedCell(row, col) {
  ...
  if (table.rows[row].cells[col].getAttribute('class') !== 'to-merge-row_' + row) {
    table.rows[row].cells[col].setAttribute('class','to-merge-row_' + row);
    //table.rows[row].cells[col].style.backgroundColor = 'green';
...
function mergeCells() {
  ...
  var mergeCellLength = document.querySelectorAll('.to-merge-row_' + row).length;
  ...
  if (isGapExistInSelectedCells()) {
    ...
    for (var i = 0; i < table.rows.length; i++) {
      for (var j = 0; j < table.rows[i].cells.length; j++) {
        document.getElementById('my-table').rows[i].cells[j].removeAttribute('class');
        document.getElementById('my-table').rows[i].cells[j].removeAttribute('style');
      }
    }
  }
}
...
function mergeDown() {
   var count = 0, spanAttr = 0, colLength = 0, colSpan = {row:[], col:[]};
   var startAtCol, startAtRow;
  for (var i=0; i< document.getElementById('my-table').rows.length; i++) {
    for (var j=0; j<document.getElementById('my-table').rows[i].cells.length; j++) {
      if (document.getElementById('my-table').rows[i].cells[j].hasAttribute('style')) {
        count = 1;
        colSpan.row.push(i);
        colSpan.col.push(j);
        if (document.getElementById('my-table').rows[i].cells[j].hasAttribute('rowspan')) {
          count = parseInt(document.getElementById('my-table').rows[i].cells[j].getAttribute('rowspan'));
        }
      } else {
        continue;
      }
  ...
  if (isRowInOrder(colSpan.row) || isSameColumn(colSpan.col)) {
    document.getElementById('my-table').rows[startAtRow].cells[startAtCol].setAttribute('rowspan',rowSpanValue);
    //document.getElementById('my-table').rows[i].deleteCell(object.startAtCol);
    document.getElementById('my-table').rows[startAtRow].cells[startAtCol].removeAttribute('class');
    document.getElementById('my-table').rows[startAtRow].cells[startAtCol].removeAttribute('style');
  //}
...
//-------------------End of merge cells vertical--------------------------------------
  ...
  function isGapExistInSelectedCells() {
    ...
    var mergeCellLength = document.querySelectorAll('.to-merge-row_' + row).length; // number of cell to merge
    for (var i = firstCellIndex; i < mergeCellLength+firstCellIndex; i++) {
    //for (var i = 0; i < document.getElementById('my-table').rows[row].cells.length; i++) {
      // Count the number of gaps between the first and last cell in the same row
      if (document.getElementById('my-table').rows[row].cells[i].getAttribute('class') !== "to-merge-row_" + row) {
        count += 1;
...
var setupEventListener = function() {
  ...
  document.querySelector('.add-preview').addEventListener('click',function(event){
  ...
  document.querySelector('.table-area').addEventListener('mousedown', function(event) {
...
/*
jQuery --- Show html code in textarea
